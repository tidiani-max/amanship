  ...
  .where(
    and(
      ...
      or(
        ...
        eq(orders.status, "arrived"),      // ← include arrived
      ),
      eq(orders.storeId, staff.storeId)
    )
  );
...
app.post("/api/driver/orders/:id/notify-nearby", async (req, res) => {
  try {
    const { id } = req.params;
    const { driverId, distanceMeters, etaMinutes } = req.body;

    if (!driverId) {
      return res.status(400).json({ error: "driverId required" });
    }

    // Guard: driver must own this order and it must be in transit
    ...
    if (!order) {
      return res.status(404).json({
        error: "Active order not found for this driver",
      });
    }

    await notifyCustomerDriverNearby(
      id,
      Math.round(distanceMeters || 0),
      Math.ceil(etaMinutes || 1)
    );

    res.json({ success: true });
  } catch (error) {
    console.error("❌ notify-nearby error:", error);
    res.status(500).json({ error: "Failed to send nearby notification" });
  }
});
// neew routes
  ...
  try {
    ...
    if (order.xenditInvoiceId) {
      ...
      try {
        ...
        if (checkResponse.ok) {
          const existingQris = await checkResponse.json();
          if (existingQris.status !== "EXPIRED") {
            console.log(`✅ Using existing valid QRIS`);
            return res.json(existingQris);
          }
        }
      } catch (e) {
        console.log(`⚠️ Could not verify existing QRIS, creating new one`);
      }
    }
    ...
    try {
      ...
      if (xenditResponse.ok) {
        ...
        if (xenditData.status === 'COMPLETED') {
          ...
          // ✅ FIX 3: Check if storeId exists before notifying
          if (order.storeId) {
...
app.post("/api/webhooks/xendit/qris", async (req, res) => {
  try {
    ...
    const { external_id, status, id: qrCodeId } = req.body;
    ...
    if (status === "COMPLETED") {
      ...
      await db
        ...
        .set({
          paymentStatus: "paid",
          qrisConfirmed: true,
          xenditInvoiceId: qrCodeId, // Store the QR code ID if not already stored
        })
        .where(eq(orders.id, order.id));

      console.log(`✅ QRIS payment confirmed for order ${order.id}`);

      // ✅ FIX: Check storeId exists before notifying
      if (order.storeId) {
        await notifyPickersNewOrder(order.storeId, order.id);
        console.log(`🔔 Pickers notified for order ${order.id}`);
      } else {
        console.error(`⚠️ Order ${order.id} has no storeId, cannot notify pickers`);
      }

      res.json({ success: true });
    } else {
      console.log(`⏳ QRIS status: ${status} for order ${external_id}`);
...
app.put("/api/driver/orders/:id/complete", async (req, res) => {
  try {
    ...
    // ── UPDATED: must tap "I've Arrived" before entering PIN ─────────────────
    if (order.status !== "arrived") {
      return res.status(400).json({
        error:
          order.status === "delivering"
            ? "Tap 'I've Arrived' before entering the customer's PIN."
            : `Order is not ready to complete (status: ${order.status}).`,
      });
    }
