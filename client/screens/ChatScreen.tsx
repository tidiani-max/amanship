...
export default function ChatScreen() {
  ...
  const { data: orderDetails } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://10.30.230.213:5000/api/orders/${orderId}`);
      return res.json();
    },
  });
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://10.30.230.213:5000/api/orders/${orderId}/messages`);
      return res.json();
    },
  ...
  mutationFn: async ({ content, type = "text" }: { content: string; type?: "text" | "image" }) => {
    ...
    const res = await fetch(`http://10.30.230.213:5000/api/messages`, {
      method: "POST",
      body: formData,
