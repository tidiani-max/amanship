    ...
    <Card style={styles.orderCard}>
      ...
      <View style={styles.orderDetails}>
        ...
        <View style={styles.detailRow}>
          ...
          <ThemedText type="body" style={{ color: '#334155' }}>{order.items.length} items to deliver</ThemedText>
        </View>
      ...
      {!isDelivered && (
        <View style={styles.orderActions}>
          <Pressable style={[styles.mapButton, { borderColor: '#e2e8f0' }]} onPress={openMaps}>
            <Feather name="navigation" size={18} color="#64748b" />
          </Pressable>

          <Pressable style={[styles.mapButton, { borderColor: '#e2e8f0' }]} onPress={() => navigation.navigate("Chat", { orderId: order.id })}>
            <Feather name="message-square" size={18} color={BRAND_PURPLE} />
          </Pressable>
          
          {isAtStore && (
            <Pressable
              style={[styles.actionButton, { backgroundColor: disabled ? "#cbd5e1" : BRAND_PURPLE, flex: 1 }]}
            ...
            >
              {isUpdating ? (
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  ...
                  <ThemedText type="button" style={{ color: "#FFF", fontWeight: '800' }}>Pick Up</ThemedText>
                </View>
              )}
            </Pressable>
          )}
          ...
          {isOnWay && (
            <Pressable
              style={[styles.actionButton, { backgroundColor: disabled ? "#cbd5e1" : BRAND_MINT, flex: 1 }]}
...
useEffect(() => {
  ...
  return (
    <ThemedView style={styles.container}>
      {/* Header */}
      <View style={[styles.header, { paddingTop: insets.top + 8, backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#f1f5f9' }]}>
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={BRAND_PURPLE} />}
      >
        ...
        {allActiveOrders.length > 0 ? (
          ...
          <Card style={styles.emptyCard}>
            ...
            <ThemedText type="h3" style={{ marginTop: Spacing.md, color: '#64748b' }}>No Deliveries</ThemedText>
            <ThemedText type="body" style={{ marginTop: 8, color: '#94a3b8' }}>New orders will appear here</ThemedText>
          </Card>
        )}
        ...
        {showCompleted && (
          <>
            {sortedCompletedOrders.length > 0 ? (
              <>
                <Card style={{ padding: 16, marginBottom: 12, backgroundColor: BRAND_MINT + "10", borderColor: BRAND_MINT + "20", borderWidth: 1 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <ThemedText type="body" style={{ fontWeight: '700', color: '#065f46' }}>💰 Total Earnings Today</ThemedText>
              ...
              <Card style={styles.emptyCard}>
                <ThemedText type="body" style={{ color: '#94a3b8' }}>No deliveries completed today</ThemedText>
              </Card>
            )}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  codBadge: { paddingHorizontal: 12, paddingVertical: 10, borderRadius: 12, marginTop: 10 },
  orderActions: { flexDirection: "row", gap: 10, marginTop: 5 },
  mapButton: { width: 48, height: 48, borderWidth: 1, borderRadius: 14, justifyContent: "center", alignItems: "center", backgroundColor: '#f8fafc' },
